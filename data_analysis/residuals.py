...
if __name__ == "__main__":
    ...
    agents = pd.read_csv('data/results/22-01-2025-agent-data.csv')
    ...
    cons_action = pd.read_csv('data/results/22-01-2025-actions.csv')
    cons_prefs = pd.read_csv('data/results/22-01-2025-preferences.csv')
    cons = pd.merge(cons_action, cons_prefs, on='p')

    # Import SLM data
    mse_data_path = "data/results/means_and_salas_molina_ps/slm_consensus.csv"
    ...
    for name, con in relevant_cons.items():
        ...
        temp_df.to_csv('data/results/normalised_residuals/principles_included_' + name + '.csv', index=False)